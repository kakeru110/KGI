...
/**
 * Sends via the property owner's own Gmail account (an App Password, not
 * the account password) rather than a transactional email service - no
 * custom domain to DNS-verify, at the cost of `from` being a gmail.com
 * address instead of a kamakuragateinn.com one. Fine at this property's
 * volume (Gmail's free-tier send limit is 500/day).
 */
...
/** Cached in-process so a warm serverless instance reuses one SMTP connection pool. */
function getTransporter(): Transporter | null {
  if (!GMAIL_APP_PASSWORD) return null;
  if (!cachedTransporter) {
    cachedTransporter = nodemailer.createTransport({
      service: "gmail",
      auth: { user: BUSINESS_INFO.email, pass: GMAIL_APP_PASSWORD },
    });
  }
...
/**
 * Notifies the owner by email when a new direct booking is created.
 * Falls back to a silent no-op if GMAIL_APP_PASSWORD or
...
export async function sendBookingNotificationEmail(params: {
  ...
  if (!transporter || !BOOKING_NOTIFICATION_EMAIL) return;

  const { bookingId, guestName, checkIn, checkOut, guests, total } = params;
  try {
    await transporter.sendMail({
      from: `"Kamakura Gate Inn" <${BUSINESS_INFO.email}>`,
 ...
 * `replyTo` so a guest hitting "reply" reaches the property management
 * company's inbox (already wired into their Slack) rather than the
 * gmail.com sending address, and `bcc`s that same inbox so they also see
 * the outbound email itself (via `bcc`, not `cc`, so the guest never sees
 * that address in the headers).
 *
